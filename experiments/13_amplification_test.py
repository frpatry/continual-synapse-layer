"""Experiment 13 — amplification variant of cold storage.

Same benchmark as experiment 12 (15-task Permuted-MNIST, dropout=0.5,
5 seeds) but adds a new ``cs_full_amplified`` method that turns on
the five opt-in amplification flags introduced into
``SynapseAugmentedMLP`` after experiment 12:

1. Multiplicative composition (amplification_alpha=1.0)
2. Confidence-weighted retrieval (confidence_exponent=0.5)
3. No-drain on consolidate (no_drain_on_consolidate=True)
4. Repeat-consolidation merging (repeat_consolidation_threshold=0.85)
5. Retrieval-success feedback (retrieval_feedback_threshold=0.9)

Methods:
- naive: control.
- cs_full: cold storage with every audit-fix gap (matches exp 12
  cs_full — fake reward, additive composition, no merging, etc.).
- cs_full_real_reward: cs_full + recentered consistency + external
  reward from per-batch accuracy (matches exp 12 cs_full_real_reward).
...
- consolidation_count, merge_count, store_count, compression sweep
  count (cumulative).
- avg_retrieved_access_count: mean access_count of the cold-storage
  entries that were active in this task's retrieval set, averaged
  across batches. Captures whether the confidence-weighted retrieval
  and the feedback bumps are actually steering toward useful entries.
- retrieval_feedback_event_count (cumulative): how many times the
  loss-below-EMA bump fired.
- modulator_gate.

Per-batch reward distribution is captured the same way as exp 12.
The output JSON also carries the full ``(T, T)`` accuracy_matrix
per (method, seed), per_seed_metrics, and pairwise Wilcoxon tables
on average_accuracy and average_forgetting (Bonferroni-corrected).

Run from the repo root::

    python experiments/13_amplification_test.py --num-tasks 15

Output: ``results/logs/amplification/<ts>_13_amplification_T15.json``.
Per-method checkpointing (atomic .tmp + os.replace) preserves the
last completed method's data on kill mid-experiment. The payload
carries ``is_partial`` plus ``methods_completed`` / ``methods_requested``
for downstream partial-vs-full detection.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Center for recentered consistency reward (real-reward variants).",
    )
    p.add_argument(
    ...
    p.add_argument("--no-drain-on-consolidate", action="store_true", default=True)
    p.add_argument("--drain-on-consolidate", dest="no_drain_on_consolidate",
                   action="store_false",
                   help="Restore the legacy drain-on-consolidate behaviour.")
    ...
    p.add_argument(
        "--output-dir",
        default=str(_REPO_ROOT / "results" / "logs" / "amplification"),
    )
    return p.parse_args()
    ...
    def _synapse(
        ...
        *,
        ...
        cold_storage = ColdStorage(
            collection_name=f"exp13_{method_key}_seed_{seed}_{time.time_ns()}",
        ...
        if amplification:
            amp_kwargs = dict(
                ...
                no_drain_on_consolidate=args.no_drain_on_consolidate,
        ...
        # Per-task accumulators reset at on_task_change start; the runner
        # calls on_task_change(i, task, model) before training task i.
        # We piggyback on on_after_batch to update; on_task_end snapshots
        # and resets via a small mutable container.
        ...
        # Send training labels (and let the model auto-derive the loss
        # from cached logits) on every method that benefits — both
        # real-reward and amplified paths use training_target.
        needs_training_target = use_real_reward or amplification
...
def _build_payload(
    ...
    *,
    ...
    return {
        "experiment": "13_amplification_test",
        "timestamp": ts,
        "config": vars(args),
...
def main() -> None:
    ...
    ts = int(time.time())
    path = output_dir / f"{ts}_13_amplification_T{args.num_tasks}.json"
    ...
    print()
    print("End-of-run amplification diagnostics (mean across seeds):")
    print(
        f"  {'method':<24s} {'consol':>8s} {'merge':>8s} "
        f"{'fb_evts':>8s} {'store':>8s} {'avg_access':>12s} {'gate':>9s}"
    )
    for method in args.methods:
        ...
        avg_access_vals = [
            f.get("avg_retrieved_access_count") for f in finals
        ]
