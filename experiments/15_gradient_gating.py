"""Experiment 15 — gradient gating from Hebbian familiarity.

Tests a new architectural hypothesis: use the synapse layer's
accumulated knowledge to GATE backprop, not to modulate output.

When the synapse layer has accumulated patterns for the current
activation (familiar pattern), reduce backprop gradients to
protect what's already learned. When the activation is novel,
allow normal backprop. Selective plasticity from Hebbian memory.

Methods (intentionally minimal: just two):
- naive: control.
- cs_gated: SynapseLayer + ColdStorage same as cs_full's, but
  - output modulator gate is set to 0 and frozen (no output
    correction at all — the synapse signal does not flow through
    to logits),
  - gradient gating is enabled (alpha=0.9): between
    ``loss.backward()`` and ``optimizer.step()``, base.parameters()
    gradients are scaled by ``1 - alpha * familiarity`` where
    ``familiarity = ||f_base @ effective_strengths||`` normalised
    by its running max.

If cs_gated shows per-task ACC trajectory where Task 0 stays
elevated as later tasks are learned, the hypothesis is validated.
Standard ACC / FGT / BWT / FWT plus per-task averages of
familiarity and gradient_scale are reported; the full ``(T, T)``
accuracy matrix per (method, seed) is preserved so the Task-0
trajectory can be inspected post-hoc.

Same 15-task PermutedMNIST × 5-seed protocol as exp 12. Per-method
JSON checkpointing at
``results/logs/gradient_gating/<ts>_15_gradient_gating_T15.json``
with atomic .tmp + os.replace.

Run from the repo root::

    python experiments/15_gradient_gating.py --num-tasks 15
"""
...
_DEFAULT_METHODS = ("naive", "cs_gated")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gradient-gating-alpha", type=float, default=0.9,
                   help="alpha in scale = 1 - alpha * familiarity. "
                        "0.9 means full-familiarity gradients reduce to 10%.")
    p.add_argument(
    ...
    p.add_argument(
        "--output-dir",
        default=str(_REPO_ROOT / "results" / "logs" / "gradient_gating"),
    )
    return p.parse_args()
...
def _build_factories(
    ...
    def cs_gated(seed: int):
        ...
        cold_storage = ColdStorage(
            collection_name=f"exp15_cs_gated_seed_{seed}_{time.time_ns()}",
        ...
        model = SynapseAugmentedMLP(
            ...
            # The core configuration for cs_gated.
        ...
        # Per-task accumulators for averages.
        ...
        def on_pre_step(i, task, m):
            ...
            task_state["familiarity_sum"] += float(m.last_familiarity)
...
def main() -> None:
    ...
    ts = int(time.time())
    path = output_dir / f"{ts}_15_gradient_gating_T{args.num_tasks}.json"
    ...
    # Task-0 retention trajectory: how does the first task's accuracy
    # evolve as later tasks are trained? The hypothesis: cs_gated keeps
    # it elevated relative to naive.
    print()
    print("Task-0 accuracy after each subsequent training task (mean ± std):")
    print(f"  {'after_task':<10s} ", end="")
    for run in runs:
        print(f"{run.method:>16s}", end=" ")
    print()
    T = args.num_tasks
    for i in range(T):
        print(f"  {i:<10d} ", end="")
    ...
    # End-of-run gating diagnostics for cs_gated (final-task snapshot).
    print()
    print("End-of-run gating diagnostics (mean across seeds, cs_gated only):")
    seed_diags = diagnostics.get("cs_gated", [])
    if seed_diags and any(d.get("per_task") for d in seed_diags):
