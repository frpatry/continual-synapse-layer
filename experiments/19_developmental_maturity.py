"""Experiment 19 — developmental maturity for gradient gating.

Tests whether ramping up gating strength as the system accumulates
consolidations (rather than firing at full strength from batch 1)
helps cs_gated_cosine. Motivated by the Split-MNIST Task-4 drop
diagnostic: at task 4, familiarity is fully saturated (~0.99) and
the gating fires at full strength (gradient scale ~0.11) yet
Task-0 still collapses ~25 pp. The hypothesis: the formula
``scale = 1 - alpha * familiarity`` is too aggressive too early —
gating fires hard even when the cold storage is small and the
"familiarity" signal is barely meaningful. Maturity scaling
addresses that by starting at no protection and growing toward
full protection as evidence accumulates.

Effective formula with maturity:

    maturity        = min(consolidation_count / target, 1)
    effective_alpha = alpha * maturity
    gradient_scale  = 1 - effective_alpha * familiarity

Methods (n=5 each, exploratory):
- naive: control.
- cs_gated_cosine: the exp 16 / exp 17 method (maturity disabled,
  i.e. target=0, equivalent to maturity always 1.0).
- cs_gated_cosine_developmental: same as cs_gated_cosine plus
  maturity_target_consolidations=50. On Permuted-MNIST (~140
  consolidations end-of-run) maturity hits 1 around the 5th task;
  on Split-MNIST multi-head (~27 consolidations end-of-run)
  maturity stays below 1 the entire run, peaking at ~0.54 at
  task 4 — i.e. the developmental variant fires the gating at
  about half strength when the Task-0 drop occurs.

Benchmarks:
- Permuted-MNIST 15 tasks, single shared 10-class head, dropout=0.5,
  1 epoch/task.
- Split-MNIST 5 tasks, multi-head (one binary head per task),
  dropout=0.5, 2 epochs/task, no zero-shot eval.

Per-task diagnostics extend exp 17 with a maturity-specific field:
- ``avg_maturity``: mean of per-batch ``last_maturity`` during the
  task. For non-developmental methods this is always 1.0; for the
  developmental variant it shows the ramp.

Per-method JSON checkpointing under
``results/logs/developmental_maturity/``. Two output files:
- ``<ts>_19_permuted_mnist_T15.json``
- ``<ts>_19_split_mnist_multihead_T5.json``

Stdout reports per-benchmark Wilcoxon (Bonferroni-corrected) on
ACC and FGT, plus per-method Task-0 retention trajectories side-
by-side. Run from the repo root::

    python experiments/19_developmental_maturity.py --seeds 0 1 2 3 4
"""
...
_METHODS = ("naive", "cs_gated_cosine", "cs_gated_cosine_developmental")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--benchmarks", nargs="+", default=["permuted_mnist", "split_mnist"],
        choices=["permuted_mnist", "split_mnist"],
    )
    # ---- Shared model / training hyperparameters (mirror exp 17) ----
    ...
    p.add_argument(
        "--maturity-target-consolidations", type=int, default=50,
        help="Target consolidation count at which gating reaches full "
             "strength. Only used by the developmental variant.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--output-dir",
        default=str(_REPO_ROOT / "results" / "logs" / "developmental_maturity"),
    )
    return p.parse_args()
...
def _build_payload(
    ...
    *,
    ...
    return {
        "experiment": "19_developmental_maturity",
...
# ---------- gating diagnostics shared by both benchmarks ----------


def _new_task_state() -> dict[str, Any]:
    return {
        "familiarity_sum": 0.0,
        "familiarity_count": 0,
...
def _build_cosine_gated(
    args, base, num_classes, *, method_key, seed, with_maturity, chroma_client
) -> tuple[SynapseAugmentedMLP, list[dict], dict[str, Any]]:
    """Construct a cs_gated_cosine model (developmental variant optional)
    plus its per-task diagnostic scaffolding."""
    ...
    cold_storage = ColdStorage(
        collection_name=f"exp19_{method_key}_seed_{seed}_{time.time_ns()}",
    ...
    model = SynapseAugmentedMLP(
        ...
        maturity_target_consolidations=(
            args.maturity_target_consolidations if with_maturity else 0
        ),
    )
    per_task_diag: list[dict] = []
    task_state = _new_task_state()
    return model, per_task_diag, task_state


# ---------- per-benchmark factory builders ----------
...
def _build_permuted_factories(
    ...
    def _cosine_factory(seed: int, *, with_maturity: bool, key: str):
        set_seed(seed)
        base = _build_mlp()
        model, per_task_diag, task_state = _build_cosine_gated(
            args, base, num_classes,
            method_key=key, seed=seed,
            with_maturity=with_maturity,
        ...
        diagnostics[key].append({"seed": seed, "per_task": per_task_diag})
        return model, runner

    return {
        "naive": naive,
        "cs_gated_cosine": lambda s: _cosine_factory(
            s, with_maturity=False, key="cs_gated_cosine"
        ),
        "cs_gated_cosine_developmental": lambda s: _cosine_factory(
            s, with_maturity=True, key="cs_gated_cosine_developmental"
        ),
    }, diagnostics
    ...
    for i in range(num_tasks):
        ...
        for run in runs:
            ...
            if vals:
                ...
                print(f"{mean:>21.3f}±{std:<7.3f}", end=" ")
            else:
                print(f"{'—':>30s}", end=" ")
        print()


def _print_per_task_maturity(
    benchmark_name: str,
    diagnostics: dict[str, list[dict]],
    num_tasks: int,
) -> None:
    print()
    print(f"[{benchmark_name}] Per-task avg_maturity (cosine methods, mean across seeds):")
    print(f"  {'task':<6s} ", end="")
    for method in ("cs_gated_cosine", "cs_gated_cosine_developmental"):
    ...
    for i in range(num_tasks):
        ...
        for method in ("cs_gated_cosine", "cs_gated_cosine_developmental"):
            if method not in diagnostics:
                continue
            vals = []
            for sd in diagnostics[method]:
                pt = sd.get("per_task", [])
                if i < len(pt) and pt[i].get("avg_maturity") is not None:
                    vals.append(float(pt[i]["avg_maturity"]))
            if vals:
                m = sum(vals) / len(vals)
                print(f"{m:>30.3f}", end=" ")
            else:
...
def main() -> None:
    ...
    benchmark_results: dict[str, tuple[list[MultiSeedRun], list, int, dict]] = {}
    ...
    if "permuted_mnist" in args.benchmarks:
        ...
        perm_path = (
            output_dir
            / f"{ts_root}_19_permuted_mnist_T{args.permuted_num_tasks}.json"
        )
        ...
        benchmark_results["permuted_mnist"] = (
            perm_runs, perm_summaries, args.permuted_num_tasks, perm_diag
        )

    # ---- Split-MNIST multi-head ----
    if "split_mnist" in args.benchmarks:
        ...
        split_path = (
            output_dir
            / f"{ts_root}_19_split_mnist_multihead_T{actual_split_tasks}.json"
        )
        ...
        benchmark_results["split_mnist"] = (
            split_runs, split_summaries, actual_split_tasks, split_diag
        )

    # ---- Side-by-side trajectory + maturity tables ----
    for name, (runs, _, T, diag) in benchmark_results.items():
        _print_task0_trajectory(name, runs, T)
        _print_per_task_maturity(name, diag, T)
